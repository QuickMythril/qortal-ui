	...
	render() {
		return html`
			...
			>
				...
				<mwc-button ?disabled='${this.isLoading}' slot='secondaryAction' @click='${() => this.hidePrivateMessageModal()}' class='close-button'>
			...
			>
				<div style='text-align:center'>
					...
					<hr>
					<h2>${translate('blockpage.bcchange6')}</h2>
					<br>
					<h2>${this.nametodialog}</h2>
				</div>
				<mwc-button slot='secondaryAction' @click='${() => this.chatBlockAddress()}' class='block'>
					${translate('general.yes')}
				</mwc-button>
				<mwc-button slot='primaryAction' @click='${() => this.hideBlockUserModal()}' class='close-button'>
